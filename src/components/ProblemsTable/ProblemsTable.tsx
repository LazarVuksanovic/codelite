    ...
    return (
        ...
        <tbody className='text-white'>
            {problems.map((problem, idx) => {
                const difficultyColor = problem.difficulty === "Easy" ? "text-dark-green-s" : problem.difficulty === "Medium" ?
                ...
                return (
                    <tr key={problem.id} className={`${idx % 2 == 1 ? 'bg-dark-layer-1' : ''}`}>
                        ...
                        <td className={'px-6 py-4'}>
                            { 
                                problem.videoId ? <AiFillYoutube fontSize={"28"} className='cursor-pointer hover:text-red-600'
        ...
        <tfoot className='fixed top-0 left-0 h-screen w-screen flex items-center justify-center'>
            <div className='bg-black z-10 opacity-70 top-0 left-0 w-screen h-screen absolute' onClick={closeModal} />
            <div className='w-full z-50 h-full px-6 relative max-w-4xl'>
                <div className='w-full h-full flex items-center justify-center relative'>
                    <div className='w-full relative'>
                        <IoClose fontSize={"35"} className='cursor-pointer absolute -top-16 right-0 hover:bg-brand-orange
                        hover:text-black rounded-lg' onClick={closeModal} />
